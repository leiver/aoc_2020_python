...
def add_adapter_to_jolt_map(adapter_to_add, jolt_map):
    for i in range(1, 4):
        if adapter_to_add - i < 0:
            break
        adapter_count = 0
        if adapter_to_add - i in adapters_for_jolt:
            adapter_count = jolt_map[adapter_to_add - i]
        adapter_count += 1
        jolt_map[adapter_to_add - i] = adapter_count


highest_adapter = 0
adapters = [0]
adapters_for_jolt = {}
with open(os.path.join(sys.path[0], "input_day10.txt"), "r") as file:
    for line in file:
