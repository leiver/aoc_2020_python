...
def bags_that_can_contain_given_bag(bag_to_traverse, containers_for_bag):
    if bag_to_traverse not in containers_for_bag:
        return []
    bag_content = containers_for_bag[bag_to_traverse]
    bags_to_return = [bag_to_return for bag_to_return in bag_content]
    for (bag_in_tree) in bag_content:
        bags_to_return = list(set(bags_to_return) | set(bags_that_can_contain_given_bag(bag_in_tree, containers_for_bag)))
...
def bags_inside_given_bag(bag_to_traverse, containers_in_bag):
    ...
    for (amount_of_bags, bag_in_tree) in bag_content:
        if amount_of_bags != "no":
            bags_inside += int(amount_of_bags) + (int(amount_of_bags) * bags_inside_given_bag(bag_in_tree, containers_in_bag))
    return bags_inside


bag_contents = {}
containers_for_bag = {}
with open(os.path.join(sys.path[0], "input_day7.txt"), "r") as file:
    for line in file:
